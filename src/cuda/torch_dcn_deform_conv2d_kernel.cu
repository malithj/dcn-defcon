...
namespace torchdcn {
inline unsigned int _GET_THREADS() {
...
inline unsigned int _GET_BLOCKS(const unsigned int THREADS,
...
__device__ float _bilinear_interpolate(const float* in, int height, int width,
                                       float h, float w) {
...
__global__ void _deformable_im2col_kernel(
    ...
    int out_h, int out_w, bool use_mask, float* columns_ptr) {
  CUDA_1D_KERNEL_LOOP(index, n) {
    ...
    for (int i = 0; i < weight_h; ++i) {
      for (int j = 0; j < weight_w; ++j) {
        ...
        *columns_ptr =
            mask_value * _bilinear_interpolate(input_ptr, height, width, y, x);
...
void _deformable_im2col(const at::Tensor& input, const at::Tensor& data_offset,
  ...
  const unsigned int threads = _GET_THREADS();
  const unsigned int blocks = _GET_BLOCKS(threads, num_kernels);

  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      input.scalar_type(), "deformable_im2col", ([&] {
        _deformable_im2col_kernel<<<blocks, threads>>>(
            num_kernels, input.data_ptr<float>(), data_offset.data_ptr<float>(),
            ...
            use_mask, data_col.data_ptr<float>());
      }));
...
int _get_greatest_divisor_below_bound(int n, int bound) {
...
__global__ void _deformable_col2im_kernel(
...
void _compute_grad_input(const at::Tensor& columns, const at::Tensor& offset,
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      columns.scalar_type(), "compute_grad_input", ([&] {
        _deformable_col2im_kernel<<<blocks, threads>>>(
...
__device__ float _get_coordinate_weight(const float* im_data, int height,
...
__global__ void _deformable_col2im_coord_kernel(
  ...
  CUDA_1D_KERNEL_LOOP(index, n) {
    ...
    for (int col_c = (offset_c / 2); col_c < c_bound; col_c += col_step) {
      ...
      const float weight =
          _get_coordinate_weight(im_ptr, height, width, y, x, is_y_direction);
      grad_offset_val += mask_value * weight * col_ptr[col_pos];

      if (use_mask && is_y_direction) {
        grad_mask_val += col_ptr[col_pos] *
                         _bilinear_interpolate(im_ptr, height, width, y, x);
      }
...
void _compute_grad_offset_and_mask(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      columns.scalar_type(), "compute_grad_offset_and_mask", ([&] {
        _deformable_col2im_coord_kernel<<<blocks, threads>>>(
...
std::tuple<at::Tensor, at::Tensor, at::Tensor> _backward_gradient_inputs(
  ...
  for (int elt = 0; elt < batch_sz / n_parallel_imgs; elt++) {
    ...
    _compute_grad_offset_and_mask(columns, input[elt], offset[elt], mask[elt],
    ...
    _compute_grad_input(columns, offset[elt], mask[elt], n_in_channels, in_h,
...
at::Tensor _backward_gradient_parameters(
  ...
  for (int elt = 0; elt < batch_sz / n_parallel_imgs; elt++) {
    _deformable_im2col(input[elt], offset[elt], mask[elt], n_in_channels, in_h,
                       in_w, weight_h, weight_w, pad_h, pad_w, stride_h,
                       stride_w, dilation_h, dilation_w, out_h, out_w,
                       n_parallel_imgs, n_offset_grps, use_mask, columns);
...
at::Tensor torch_warp_deform_conv2d_forward_kernel(
  ...
  int n_parallel_imgs =
      _get_greatest_divisor_below_bound(batch_sz, kMaxParallelImgs);
  ...
  for (int b = 0; b < batch_sz / n_parallel_imgs; b++) {
    _deformable_im2col(input_c[b], offset_c[b], mask_c[b], in_channels, in_h,
                       in_w, weight_h, weight_w, pad_h, pad_w, stride_h,
                       stride_w, dilation_h, dilation_w, out_h, out_w,
                       n_parallel_imgs, n_offset_grps, use_mask, columns);
...
_torch_warp_deform_conv2d_backward_kernel(
  ...
  auto grad_input_and_offset_and_mask = _backward_gradient_inputs(
  ...
  auto grad_weight = _backward_gradient_parameters(
      input_c, weight_c, offset_c, mask_c, grad_out_c, stride_h, stride_w,
      pad_h, pad_w, dilation_h, dilation_w, n_weight_grps, n_offset_grps,
      n_parallel_imgs, use_mask);
